...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(['Index & ETF Performances',
                                                    'Pure S&P',
                                                    'Rebalancing 60/40',
...
with tab8:
    ...
    st.markdown('# Using the VIX as an Indicator')
    st.markdown('### VIX Data')
    st.dataframe(VIX_and_10_Year)
    ...
    fig.add_trace(go.Scatter(
        x = dates,
        y = vix_30_day_mov_avg,
        mode = 'lines',
        name = 'VIX_(MOV[30])',
        line = dict(color = 'teal')
    ))
    
    fig.add_hline(y = 25, line_width = 1, line_color = 'red', line_dash = 'dash')
    
    fig.update_layout(
        title = 'Performance of VIX over Early 2000s Recession',
        xaxis_title = 'Date',
        yaxis_title = 'VIX Values',
        showlegend = True,
    ...
    st.markdown('### Early 2000s Recession')
    st.plotly_chart(fig)
    ...
    fig.update_layout(
        title = 'Performance of VIX over Great Recession',
        xaxis_title = 'Date',
        yaxis_title = 'VIX Values',
        showlegend = True,
        xaxis = dict(tickformat='%m-%d-%Y'),
    ...
    fig.update_layout(
        title = 'Performance of VIX over COVID-19 Recession',
        xaxis_title = 'Date',
        yaxis_title = 'VIX Values',
        showlegend = True,
        xaxis = dict(tickformat='%m-%d-%Y'),
    ...
    st.markdown('### COVID-19 Recession')
    st.plotly_chart(fig)
...
with tab9:
    ...
    st.markdown('# Applying indicator to SAA Strategy')
    ...
    # Create empty lists to store the segments
    x_saa = []
    y_saa = []
    x_ray_dalio = []
    y_ray_dalio = []

    # Iterate through each row and separate the data based on 'Strategy_B'
    for i in range(len(VIX_DF) - 1):
        if VIX_DF.loc[VIX_DF.index[i], 'Strategy_B'] == 0:
            x_saa.extend([VIX_DF.iloc[i]['Dates'], VIX_DF.iloc[i+1]['Dates'], None])
            y_saa.extend([VIX_DF.iloc[i]['VIX Index 10_day_moving_average'], VIX_DF.iloc[i+1]['VIX Index 10_day_moving_average'], None])
        else:
            x_ray_dalio.extend([VIX_DF.iloc[i]['Dates'], VIX_DF.iloc[i+1]['Dates'], None])
            y_ray_dalio.extend([VIX_DF.iloc[i]['VIX Index 10_day_moving_average'], VIX_DF.iloc[i+1]['VIX Index 10_day_moving_average'], None])

    # Create the traces
    trace_saa = go.Scatter(
        x = x_saa,
        y = y_saa,
        mode = 'lines',
        line = dict(color='yellowgreen'),
    ...
    trace_ray_dalio = go.Scatter(
        x = x_ray_dalio,
        y = y_ray_dalio,
        mode = 'lines',
        line = dict(color='forestgreen'),
        name  ='Ray Dalio Portfolio'
    )

    # Create the figure
    fig = go.Figure(data = [trace_saa, trace_ray_dalio])

    # Customize the layout
    fig.update_layout(
        title = 'Portfolio Strategy Using VIX Indicator',
        xaxis_title = 'Date',
        yaxis_title = 'VIX Index 10 Day Moving Average',
        showlegend = True,
        width = 1200,
        height = 600
    )

    st.markdown('### Strategy Overlay on VIX')
    st.plotly_chart(fig)
    ...
    for i in range(len(strategy_visualization_data) - 1):
        if strategy_visualization_data.loc[strategy_visualization_data.index[i], 'Strategy_B'] == 0:
            x_saa.extend([strategy_visualization_data.iloc[i]['Dates'], strategy_visualization_data.iloc[i+1]['Dates'], None])
            y_saa.extend([strategy_visualization_data.iloc[i]['Rebalanced SAA Portfolio Value'], strategy_visualization_data.iloc[i+1]['Rebalanced SAA Portfolio Value'], None])
        else:
            x_ray_dalio.extend([strategy_visualization_data.iloc[i]['Dates'], strategy_visualization_data.iloc[i+1]['Dates'], None])
            y_ray_dalio.extend([strategy_visualization_data.iloc[i]['Rebalanced SAA Portfolio Value'], strategy_visualization_data.iloc[i+1]['Rebalanced SAA Portfolio Value'], None])
    ...
    fig.update_layout(
        title = 'Portfolio Strategy Using VIX Indicator',
        xaxis_title = 'Date',
        yaxis_title = 'Rebalanced SAA Portfolio Value',
        showlegend = True,
        width = 1200,
        height = 600
    )

    st.markdown('### Indicator Overlay on SAA')
    st.plotly_chart(fig)
